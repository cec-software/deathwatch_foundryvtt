(async () => {
  ...
  const regionData = {
    name: "Heavy Flamer Template",
    shapes: [{
      type: "cone",
      x: token.center.x,
      y: token.center.y,
      radius: (30 / (canvas.grid.distance || 3)) * (canvas.grid.size || 100),
      angle: 30,
  ...
  ui.notifications.info("Heavy Flamer template placed - adjust as needed");
})();
